...
for i in "${!TASK[@]}"; do
    ...
    for j in {0..3}; do
        ...
        COMMAND="python main.py --run_group=${TASK[$i]} --env_name=${TASK[$i]} --offline_steps=500000 --agent=agents/ifql.py --agent.num_samples=32 \
        ...
        # Task: ${TASK[$i]} \
        # Seed: ${SEEDS[$j]}'" C-m
        # python main.py --env_name="visual-cube-single-play-singletask-task2-v0" --offline_steps=500000 --agent=agents/ifql.py --agent.num_samples=32 --agent.encoder=impala_small --p_aug=0.5 --frame_stack=3 --device_id=2 --mujoco_device_id=3 --seed=1234
    done
done
